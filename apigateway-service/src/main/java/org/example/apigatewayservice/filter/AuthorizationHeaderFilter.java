...
public class AuthorizationHeaderFilter extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter.Config> {
    ...
    private Mono<Void> onError(final ServerWebExchange exchange, final String err, final HttpStatus httpStatus) {
        ...
        final byte[] bytes = "The requested token is invalid.".getBytes(StandardCharsets.UTF_8);
    ...
    private boolean isJwtValid(final String jwt) {
        ...
        final byte[] secretKeyBytes = env.getProperty("token.secret").getBytes();
        final SecretKey signingKey = new SecretKeySpec(secretKeyBytes, SignatureAlgorithm.HS512.getJcaName());
